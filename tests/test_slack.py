...
def test_signing_secret():
    ...
    assert (
        verify_token(fake_request_headers, fake_request_body, "faultyfaketoken")
        is not True
    )
    fake_test_token = "8f742231b10e8888abcd99yyyzzz85a5"
    assert (
        verify_token(fake_request_headers, fake_request_body, fake_test_token) is True
    )
...
def test_slack_client_responder():
    ...
    when(requests).post(url=fake_url, json=fake_data, headers=fake_headers).thenReturn(
        mock({"status_code": 200, "text": '{"ok": "true", "message_ts": "xxxxx"}'})
    )

    test_result = slack_client_responder(
        token="fake", user_id="fake", attachment="fake", url=fake_url
    )
...
def test_delete_menu():
    test_result = delete_message_menu(user_name="fake user", date="2018-01-01")
